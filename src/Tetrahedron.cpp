...
VectorXd Tetrahedron::computeCorotationalElasticForces(VectorXd f) {
	this->F = computeDeformationGradient();
	this->R = gs3(this->F);
	...
	//cout << "Ke" << this->Ke << endl;

	// Forces
	this->RKxx = this->Re * this->Kexx;
	//cout << "RKX" << this->RKxx << endl;
	for (int i = 0; i < 4; ++i) {
	...
	for (int i = 0; i < 4; i++) {
		...
		//cout << "f" << f.segment<3>(rowi) << endl;

	}
	return f;
}

void Tetrahedron::computeCorotationalForceDifferentials(MatrixXd &K_global) {
	int i = m_nodes[0]->idxM;
	int j = m_nodes[1]->idxM;
	int k = m_nodes[2]->idxM;
	int l = m_nodes[3]->idxM;
...
void Tetrahedron::computeCorotationalForceDifferentialsSparse(vector<T> &K_) {
